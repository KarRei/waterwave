...
//för eulers metod så åste iterationen dt specificeras
var MAX_DT = 12;
...
//sätter igång den första droppen
...
integrate = function(dt) {
  ...
  v = geometry.vertices;
  for (z = 1; z < N; z++) { //; 1 <= N ? z < N : z > N; z = 1 <= N ? ++z : --j) {
    for (x = 1; x < N; x++) { //k = 1, ref1 = N; 1 <= ref1 ? k < ref1 : k > ref1; x = 1 <= ref1 ? ++k : --k) {
      i = idx(x, z);
      iPrevX = idx(x - 1, z);
      iNextX = idx(x + 1, z);
      iPrevZ = idx(x, z - 1);
